...
## 根本原因
难度筛选状态的恢复代码只在 `loadLastSession()` 方法中执行，而这个方法：
1. 只在有保存的会话时才会执行
2. 只在成功加载题库后才会恢复筛选状态
3. 如果用户第一次打开程序或没有会话，筛选状态就不会被恢复
...
将难度筛选状态的恢复移到构造函数中，确保无论是否有会话都能恢复：
...
## 修改的文件
1. `src/ui/MainWindow.cpp` - 将难度筛选恢复移到构造函数，移除重复代码
