...
2. 提取信息：
   - 标题：从原文提取
   - 难度：根据题目复杂度判断（简单/中等/困难）
   - **描述：完整复制原文的题目描述，包括所有输入格式、输出格式说明**
   - 标签：根据题目内容判断（如数组、字符串、动态规划等）
3. 生成至少5组测试用例：
   - 基本功能测试（2-3个）
   - 边界条件（空输入、最小值、最大值）
   - 特殊情况（负数、零、重复、无解）
...
❌ 错误示例：
description: "给定一个数组，找出两数之和" （这是改写！）
input: "100 1\n1 0\n...（重复98行）\n0 0"

✅ 正确示例：
description: "【原文完整内容】给定一个整数数组 nums 和一个整数目标值 target，请你在该数组中找出和为目标值 target 的那两个整数，并返回它们的数组下标。\n\n输入格式：\n第一行包含两个整数 n 和 target...\n\n输出格式：\n输出两个整数..." （完整保留原文）
input: "5 1\n1 0\n1 0\n1 0\n1 0\n0 0"
